...
```
   ...
   0x080488d5 <+123>:   call   0x8048747 <test>
```
> Prompt stdin and call `test` with the input as first argument and the number 0x1337d00d as second argument.
...
```
   ...
   0x08048852 <+267>:   call   0x8048660 <decrypt>
```
> Subtract our input from 0x1337d00d, store the result in the stack, if the result is greater than 0x15, call `0x8048660 <decrypt>` with the return of `0x8048520 rand@plt` as the first parameter.

- So if our input is lower than 0x1337d00d - 0x15, we will call `decrypt` with a random value as the first parameter, which I believe is not what we want.

- Afterwards, we see the following pattern repeated 15 times: 
```
   0x08048785 <+62>:    mov    -0xc(%ebp),%eax
   0x08048788 <+65>:    mov    %eax,(%esp)
   0x0804878b <+68>:    call   0x8048660 <decrypt>
...
- So, depending of our input, we are going to jump into one of these 15 blocks, or into the block which call `0x8048660 <decrypt>` with the return of `0x8048520 rand@plt`.

- In this context, at this point, no matter what the rest of the code does, there are only 16 possible outcomes, and we already know that the password must be a number between 0x1337d00d and 0x1337d00d - 0x15.
So we can create a simple script that will find it quickly.
```sh
