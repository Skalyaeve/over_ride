...
> `fopen@plt` is called with our flag as first argument.
...
```
   ...
   0x0000000000400901 <+237>:   callq  0x400690 <fread@plt>
```
> Our flag is stored at `-0xa0(%rbp)`.
...
```
   ...
   0x00000000004009d9 <+453>:   callq  0x4006f0 <fgets@plt>
```
> `<fgets@plt>` is called for 0x64 bytes with a buffer located at `-0x70(%rbp)`.
...
```
   ...
   0x0000000000400aa2 <+654>:   callq  0x4006c0 <printf@plt>
```
> In the case the password doesn't match our flag, `<printf@plt>` is called with our buffer as first argument.

- Well as `<printf@plt>` is called with our buffer as first argument and our flag is actually stored in the stack, we can simply print the stack using printf with `%lx`.
...
- 44434241 is the hex value of ABCD, it is the beginning of our buffer. What we can see just before the the beginning of our buffer is actually our flag because our buffer is stored right after our flag in the stack (`-0x70(%rbp)` is the address of beginning of our buffer and `-0xa0(%rbp)` is is the address of the beginning of our flag).

> 756e505234376848 --> unPR47hH (little endian) --> Hh74RPnu

> 45414a3561733951 --> EAJ5as9Q (little endian) --> Q9sa5JAE

> 377a7143574e6758 --> 7zqCWNgX (little endian) --> XgNWCqz7

> 354a35686e475873 --> 5J5hnGXs (little endian) --> sXGnh5J5

> 48336750664b394d --> H3gPfK9M (little endian) --> M9KfPg3H
