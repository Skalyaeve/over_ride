...
```
   ...
   0x080484ed <+29>:    rep stos %eax,%es:(%edi)
```
> Fill a buffer of 0x10 * 4 bytes on the stack with 0x0.
...
> Prompt stdin for 0x100 bytes, store the input in a buffer declared in the bss section, and then call the `<verify_user_name>` function.
...
> If the user input is "dat_wil", return 0.
...
```
   ...
   0x0804854e <+126>:   jmp    0x80485af <main+223>
```
> If `<verify_user_name>` does not return 0, display a message and return 1.
...
```
> Prompt stdin for 0x64 bytes, store the input in a 0x40 byte buffer declared on the stack, and then call the `<verify_user_name>` function.
...
```
   ...
   0x080485a8 <+216>:   jmp    0x80485af <main+223>
```
> Regardless of what happens next, we display an error message and return 1.

- It `<fgets>` 0x64 bytes into a buffer of 0x40 bytes. If the user input is large enough, the return address of the main function can be overwritten.So, we will redirect the program's execution flow to the beginning of our buffer. We will use the shellcode that we built during Rainfall.
```
